...
- 2026-03-31 01:05 Europe/London: The Stage 4 Python service now exposes `/api/stage4/board-create-candidates`, `/api/stage4/board-create-guess/<unitId>`, and `POST /api/stage4/board-create-from-unit` for discovery-assisted board creation.
- 2026-03-31 01:05 Europe/London: The first-guess board flow clones a matched or candidate board definition when possible, and otherwise falls back to a small generic controller-based draft using the observed chip family and profile evidence.
- 2026-03-31 01:05 Europe/London: A controlled create-and-cleanup validation succeeded for the unresolved COM7 ESP32 board, producing a temporary draft board with three inherited signals and no buses before cleanup and tree regeneration.
...
- 2026-03-31 11:33 Europe/London: Ran per-unit validation one card at a time with explicit 3 second capture windows for the four matched attached boards: COM3 Dial, COM4 CoreS3+GNSS, COM5 Dial, and COM6 P-NUCLEO / Nucleo-F072RB. COM7 remains discovery-only because it still has no matched board definition.
- 2026-03-31 11:46 Europe/London: Checked the Dial and CoreS3 platform init paths against the generated boot order and current board mappings. No obvious UART mapping bug was found. The Dial validation gap was mostly missing recurring I2C-scan output rather than absent I2C setup.
- 2026-03-31 12:18 Europe/London: The attached F072 board platform is no longer GPIO-only. `project/platform/stm32cube/p_nucleo_usb001_f072rb_v1_platform.c` now initializes GPIO, I2C1 on PB8/PB9 for the modeled `usbpd_i2c` bus, and a controller-side SPI2 master scaffold on PB13/PB14/PB15 so the STM32 demo has concrete GPIO/I2C/SPI bring-up code.
- 2026-03-31 13:26 Europe/London: The persisted board layer was intentionally reset at user request. All board JSON definitions under `project/boards`, generated board artifacts under `project/generated`, board help pages under `project/help/boards`, board validation contracts under `project/job-manager/contracts`, board validation reports under `project/job-manager/reports`, and the Stage 4 tree snapshot were removed so board identification can restart from the changed hardware state.
- 2026-03-31 14:25 Europe/London: The attached hardware has been reduced to one plain M5Stack CoreS3 with the GNSS module removed. New module-level tests now live with the reusable parts instead of depending on concrete board definitions: project/parts/mcu/esp32_s3/tests/hello_world, project/parts/mcu/esp32_s3/tests/read_fuse_data, and project/parts/devices/sdcard_spi/tests/read_card_info.
- 2026-03-31 14:25 Europe/London: The CoreS3 SD-card slot test uses the official M5Stack CoreS3 SPI pin map: CS GPIO4, SCK GPIO36, MISO GPIO35, MOSI GPIO37.
- 2026-03-31 15:05 Europe/London: Module-level tests now keep per-part run logs in the part test folders. The first logs are project/parts/mcu/esp32_s3/tests/run-log.md and project/parts/devices/sdcard_spi/tests/run-log.md, recording expected result versus observed result per run.
