...
- 2026-03-31 10:52 Europe/London: The Stage 3 validation runner must use a filesystem-resolved `pathToFileURL(path.resolve(process.argv[1]))` entrypoint check so direct `node <script>` launches work reliably on Windows.
- 2026-03-31 10:52 Europe/London: Stage 4 board validation should always emit a runner log file, even when the validation subprocess produces no JSON payload, so the UI can surface a real diagnostic path instead of a generic `exit code 0` message.
- 2026-03-31 11:46 Europe/London: Treat missing recurring `BoardManagerI2CScan` output as a validation-evidence bug in the firmware app, not as proof that the I2C pins are wrong. The Dial app should emit scan output continuously during the validation window, just like the CoreS3 app.
- 2026-03-31 11:46 Europe/London: Treat the CoreS3 `BoardManagerI2CScan: observed=none` plus ESP-IDF `I2C software timeout` messages as a real bus/probe issue worth separate hardware or low-level driver investigation, not as a web/UI issue.
- 2026-03-31 12:18 Europe/London: For the attached F072 board, put the real modeled bus on I2C1 PB8/PB9 and keep SPI setup on a non-conflicting controller-side SPI2 scaffold on PB13/PB14/PB15 instead of reusing PA5/PA6/PA7, because those pins are already assigned to board outputs in the board definition.
- 2026-03-31 12:18 Europe/London: On STM32CubeF0, treat DMA HAL enablement as part of enabling the I2C/SPI HAL drivers for this project. The F0 I2C/SPI drivers depend on DMA types even when the current platform code does not actively configure DMA channels.
- 2026-03-31 13:26 Europe/London: Treat the user-requested hardware restart as a full persisted-board reset, not just a rediscovery pass. Delete the board definitions and board-derived artifacts first so no stale board metadata can influence the next read/test cycle.
- 2026-03-31 13:26 Europe/London: Before deleting persisted board data, restore any half-finished board-specific code changes from the interrupted CoreS3 bring-up attempt so the reset commit reflects only the board-layer reset and not mixed unfinished firmware work.
- 2026-03-31 14:25 Europe/London: Store reusable hardware tests with the reusable part they exercise instead of forcing early recreation of board definitions. For this restart, the first CoreS3 checks live under project/parts/mcu/esp32_s3/tests and project/parts/devices/`sdcard_spi`/tests as standalone module tests.
