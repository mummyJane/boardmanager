...
const schemaPath = path.join(projectRoot, "job-manager", "schema", "validation-report.schema.json");
...
function validateValue(schema, value, context, errors) {
  ...
  if (schema.required && value && typeof value === "object" && !Array.isArray(value)) {
    for (const key of schema.required) {
      if (!(key in value)) errors.push(`${context}: missing required property '${key}'`);
    }
  }
  if (schema.additionalProperties === false && value && typeof value === "object" && !Array.isArray(value) && schema.properties) {
    const allowed = new Set(Object.keys(schema.properties));
    for (const key of Object.keys(value)) {
      if (!allowed.has(key)) errors.push(`${context}: unexpected property '${key}'`);
    }
  }
  ...
  if (schema.properties && value && typeof value === "object" && !Array.isArray(value)) {
    for (const [key, propertySchema] of Object.entries(schema.properties)) {
      if (key in value) validateValue(propertySchema, value[key], `${context}.${key}`, errors);
    }
  }
  if (schema.items && Array.isArray(value)) {
    for (const [index, item] of value.entries()) validateValue(schema.items, item, `${context}[${index}]`, errors);
  }
}
...
async function main() {
  ...
  console.log(`Validated Stage 3 validation reports: ${files.length} report files.`);
}
