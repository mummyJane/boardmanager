...
async function runEspTool(port) {
  try {
    const { stdout, stderr } = await execFileAsync(espPython, [esptoolPy, "--chip", "esp32s3", "-p", port, "chip_id", "read_mac"], {
      cwd: repoRoot,
      windowsHide: true,
      maxBuffer: 1024 * 1024,
    });
...
async function captureSignature(port) {
  ...
  try {
    ...
    if (rawSignatureLine) {
      if (rawSignatureLine.includes("Live GNSS PPS state:")) firmwareSignature = "board_manager_cores3_gnss_demo";
...
function matchUnit(unit) {
  ...
  const { mac, firmwareSignature } = unit.observed;
...
async function observePort(portInfo) {
  const transport = {
    kind: "serial",
  ...
  if ((vidPid.vid === "303A" && vidPid.pid === "1001") || (transport.name ?? "").includes("USB Serial Device")) {
    ...
    observed.mac = parseMac(toolOutput);
...
async function main() {
  ...
  for (const portInfo of ports) {
    units.push(await observePort(portInfo));
  }
  ...
  for (const unit of units) {
    console.log(`${unit.transport.port}: ${unit.match.boardId ?? "unmatched"} (${unit.match.reason})`);
  }
}
